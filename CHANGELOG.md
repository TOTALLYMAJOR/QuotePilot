...
Last updated: 2026-09-03 19:46:34 CDT
...
### Changed
- Added a state-aware configured-quote action compiler and migrated the quote administration row onto one ranked primary/recovery action while grouping subordinate capabilities under **More**. Draft edits now read **Edit draft**, sent/viewed changes read **Revise quote**, tracked provider dispatch reads **Send proposal**, provider acceptance is evidence rather than a disabled command, and payment/contract/recovery actions use outcome language. **Create alternate draft** now declares its separate-record consequence before creation, opens the created draft, and the browser-local fallback uses an explicit presentation/commercial allowlist while clearing acceptance, rebooking, delivery, payment-provider, integration-receipt, and booking proof instead of cloning the source object. The quote number now opens its workspace, Firebase lifecycle management exposes explicit **Expire quote** instead of a generic selector, and exact workflow deep links open **More** before focusing a secondary approved action. Final-balance ranking now mirrors the existing booked-contract + verified paid-deposit + stored-balance prerequisites, and sent/viewed records without current delivery evidence recover through tracked resend instead of conversation. Existing Firebase, Stripe, portal, approval, and contract authority is unchanged; local/CI evidence does not claim hosted, provider, production, or human acceptance.
...
- Reconciled the production-equivalent Ambient bundle exception to the exact
  ...
  build measured 3,928,552; the temporary ceiling is pinned to the larger
  literal result with no discretionary growth headroom. The unchanged
  391,901-byte largest-chunk ceiling still contains both measured graphs.
- Repaired the Ambient zero-dead-click release gate after exact remote CI
...
- Added one fail-closed canonical browser-arrival protocol for quote-scoped
  **Payment**, **Proposal**, and general **Conversation** intent. Payment and
  ...
  semantic-object matching; Conversation preserves the exact quote-scoped
  communication object. The contract strips caller-authored display copy,
  rejects substituted objects, records, intents, or routes, and grants no
  payment, proposal, message, quote, delivery, booking, or provider authority.
- Kept the normalized quote-store default `followUp.stage = "new"` shell from
  becoming scheduled work. A Conversation object now requires a due date, note,
  completion, non-default stage, actor, or update timestamp before follow-up can

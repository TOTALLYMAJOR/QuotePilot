...
function makeReceiptArgs(overrides = {}) {
  const target = String(overrides.target || "vercel");
  const applicableItemIds = checklist.itemIdsByTarget[target]
    || checklist.itemIdsByTarget.vercel;
  return {
    ...
    "checked-item-ids": applicableItemIds.join(","),
...
describe("tracked UAT checklist", () => {
  test("loads a versioned, target-scoped checklist and stable digest", () => {
  ...
  test.each([
    ...
    [{ schema: "wrong", version: "fixture", maximumAttestationAgeHours: 24, items: [] }, /schema is not supported/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: 2, maximumAttestationAgeHours: 24, items: [] }, /version is required/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [] }, /no required items/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "Bad ID", label: "Bad", targets: ["firebase-all"] }] }, /invalid or duplicate item ids/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "same", label: "One", targets: ["firebase-all"] }, { id: "same", label: "Two", targets: ["firebase-all"] }] }, /invalid or duplicate item ids/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "", targets: ["firebase-all"] }] }, /invalid label/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: 1, targets: ["firebase-all"] }] }, /invalid label/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "One" }] }, /unsupported fields/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "One", targets: [] }] }, /invalid target applicability/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "One", targets: ["firebase-all", "firebase-all"] }] }, /invalid target applicability/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "One", targets: ["firebase-functions", "firebase-all"] }] }, /invalid target applicability/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "One", targets: ["firebase-hosting", "vercel"] }] }, /must apply to firebase-all/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "One", targets: ["firebase-all"] }] }, /cannot apply only to the combined Firebase target/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "One", targets: ["firebase-hosting", "firebase-backend", "firebase-all"] }] }, /leaves a deployment target without required items/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "One", targets: ["firebase-all"], ignored: true }] }, /unsupported fields/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 24, items: [{ id: "one", label: "One", targets: ["firebase-all"] }], ignored: true }, /unsupported fields/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 0, items: [{ id: "one", label: "One", targets: ["firebase-hosting", "firebase-backend", "firebase-all", "vercel"] }] }, /invalid attestation age limit/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: "24", items: [{ id: "one", label: "One", targets: ["firebase-hosting", "firebase-backend", "firebase-all", "vercel"] }] }, /invalid attestation age limit/i],
    [{ schema: "com.mbmapps.quotepilot.release-uat-checklist/v2", version: "fixture", maximumAttestationAgeHours: 169, items: [{ id: "one", label: "One", targets: ["firebase-hosting", "firebase-backend", "firebase-all", "vercel"] }] }, /invalid attestation age limit/i]
...
describe("release UAT attestation validator", () => {
  ...
  test.each(DEPLOYMENT_PROFILES)(
    ...
    (profile) => {
      ...
      expect(receipt.checklist.checkedItemIds).toEqual(
        EXPECTED_UAT_ITEM_IDS_BY_TARGET[profile]
      );
  ...
  test.each([
    ...
    [{ "checked-item-ids": checklist.itemIdsByTarget.vercel.slice(1).join(",") }, {}, /applicable to vercel exactly once/i],
    [{ "checked-item-ids": [...checklist.itemIdsByTarget.vercel, checklist.itemIdsByTarget.vercel[0]].join(",") }, {}, /applicable to vercel exactly once/i],
    [{ "checked-item-ids": [...checklist.itemIdsByTarget.vercel, "quote.authoritative-create"].join(",") }, {}, /applicable to vercel exactly once/i],
  ...
  test.each(DEPLOYMENT_PROFILES)(
    "rejects missing, duplicate, and inapplicable ids for %s",
    (profile) => {
      const expected = checklist.itemIdsByTarget[profile];
      const inapplicable = checklist.itemIds.find((itemId) => !expected.includes(itemId));
      const invalidLists = [
        expected.slice(1),
        [...expected, expected[0]]
      ];
      if (inapplicable) invalidLists.push([...expected, inapplicable]);

      for (const checkedItemIds of invalidLists) {
        expect(() => buildReleaseUatReceipt(makeReceiptArgs({
          target: profile,
          "checked-item-ids": checkedItemIds.join(",")
        }), {
          env: makeReceiptEnv(),
          root: process.cwd(),
          now: NOW
        })).toThrow(new RegExp(`applicable to ${profile} exactly once`, "i"));
      }
    }
  );

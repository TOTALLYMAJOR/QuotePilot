...
describe("configured quote action state", () => {
  test("makes tracked proposal delivery the primary action for a saved draft", () => {
    ...
    expect(result.modelId).toBe("configured-quote-action-state-v2");
  ...
  test("ranks an executable accepted-quote outcome ahead of a blocked preferred outcome", () => {
    const currentQuote = quote("accepted", {
      workflow: {
        quoteDelivery: providerAcceptedDelivery(),
        approvalRequests: [
    ...
    const result = buildRoleSafeQuoteActionController({
      quote: currentQuote,
      ...
      runtimeActions: {
        convert_contract: {
          enabled: false,
          disabledReason: "Contract conversion is already in progress."
        },
        request_deposit: { enabled: true }
      }
    }).actionState;

    expect(result.actions.convert_contract).toMatchObject({
      visible: true,
      enabled: false,
      disabledReason: "Contract conversion is already in progress."
    });
  ...
  test("includes surface readiness in the compiled action instead of diverging in the renderer", () => {
    const result = buildRoleSafeQuoteActionController({
      quote: quote("draft"),
      currentUserRole: "admin",
      source: "firebase",
      runtimeActions: {
        send_quote: {
          enabled: false,
          disabledReason: "Configure a supported email provider in Integration Ops first."
        }
      }
    }).actionState;

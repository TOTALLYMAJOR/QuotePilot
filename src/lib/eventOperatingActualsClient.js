...
function normalizeSnapshot(value, requested) {
  ...
  if (new Set(entries.map((entry) => entry.entryId)).size !== entries.length) fail("Actuals contain duplicate entries.");
  ...
  const categories = Object.fromEntries(EVENT_ACTUALS_CATEGORIES.map((category) => {
    ...
    if (item.state === "not_declared" && entries.some((entry) => entry.category === category)) fail("Retained actuals cannot have an undeclared category.");
    if (["complete", "not_applicable"].includes(item.state) && entries.some((entry) => entry.category === category && Date.parse(entry.updatedAtISO) > Date.parse(item.declaredAtISO))) fail("Actuals completeness predates its retained entries.");
    if (item.state === "not_applicable" && entries.some((entry) => entry.category === category && entry.state === "active")) fail("Not-applicable actuals contradict active entries.");
  ...
  if (value.captureComplete !== captureComplete) fail("Actuals completeness does not match its declarations.");
  ...
  if (Object.keys(expected).some((field) => !integer(value.totals[field]) || value.totals[field] !== expected[field])) fail("Captured actuals totals contradict their recorded entries.");
  let latestReceipt = null;
  if (value.availability === "not_yet_available") {
    if (!["phase_ledger_missing", "actuals_empty"].includes(value.reasonCode) || value.revision !== 0 || entries.length || captureComplete || EVENT_ACTUALS_CATEGORIES.some((category) => categories[category].state !== "not_declared") || value.latestReceipt !== null || value.lastReceiptId !== "" || value.updatedAtISO !== "") fail("Absent actuals contain contradictory recorded evidence.");
  } else if (value.availability === "available") {
    latestReceipt = receipt(value.latestReceipt);
    if (value.reasonCode !== "" || value.revision < 1 || value.lastReceiptId !== latestReceipt.receiptId || value.updatedAtISO !== latestReceipt.recordedAtISO || value.revision !== latestReceipt.resultRevision) fail("The latest actuals receipt does not establish this journal.");
    if (entries.some((entry) => Date.parse(entry.updatedAtISO) > Date.parse(value.updatedAtISO)) || Object.values(categories).some((item) => item.declaredAtISO && Date.parse(item.declaredAtISO) > Date.parse(value.updatedAtISO))) fail("Actuals evidence is newer than its journal snapshot.");
    if (latestReceipt.command === "declare_category") { const item = categories[latestReceipt.category]; if (item.lastDeclarationReceiptId !== latestReceipt.receiptId || item.declaredAtISO !== latestReceipt.recordedAtISO) fail("The latest actuals declaration is contradictory."); }
    else { const entry = entries.find((item) => item.entryId === latestReceipt.entryId); if (!entry || entry.category !== latestReceipt.category || entry.state !== (latestReceipt.command === "void" ? "voided" : "active") || entry.lastReceiptId !== latestReceipt.receiptId || entry.updatedAtISO !== latestReceipt.recordedAtISO || categories[entry.category].state !== "partial") fail("The latest actuals entry is contradictory."); }
...
function envelope(value, requested, mutation = false) {
  ...
  if (value.ok !== true || value.storage !== "firebase" || value.organizationId !== requested.organizationId || value.quoteId !== requested.quoteId) fail("The actuals response belongs to another event.");

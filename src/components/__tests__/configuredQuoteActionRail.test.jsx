...
describe("ConfiguredQuoteActionRail", () => {
  test("keeps one ranked action outside More while grouping subordinate capabilities", () => {
      ...
      <ConfiguredQuoteActionRail
        ...
        actions={{
          send_quote: {
            ...
            visible: true,
            enabled: true,
            consequence: "Send the current saved revision."
          },
          duplicate: { id: "duplicate", visible: true, enabled: true },
          export_proposal: { id: "export_proposal", visible: true, enabled: true }
        }}
      >
    ...
    expect(markup).toContain('data-configured-quote-actions="v2"');
  ...
  test("shows the compiler blocker rather than inventing a substitute primary action", () => {
    const markup = renderToStaticMarkup(
      <ConfiguredQuoteActionRail
        ...
        actions={{ duplicate: { id: "duplicate", visible: true, enabled: true } }}
      >
        <button
          type="button"
          data-quote-action-id="duplicate"
  ...
  test("fails closed for unmanaged actions and enforces compiled disabled state", () => {
    const markup = renderToStaticMarkup(
      <ConfiguredQuoteActionRail
        actions={{
          send_quote: {
            ...
            visible: true,
            enabled: false,
            disabledReason: "Configure delivery first."
          }
        }}
      >
        <button type="button" data-quote-action-id="send_quote" data-quote-action-group="communication">
          Send proposal
        </button>
        <button type="button" data-quote-action-id="unmanaged" data-quote-action-group="other">
          Unmanaged action
        </button>
      </ConfiguredQuoteActionRail>
    );

    expect(markup).toContain('data-quote-action-state="blocked"');
    expect(markup).toContain('aria-disabled="true"');
    expect(markup).toContain('title="Configure delivery first."');
    expect(markup).not.toContain("Unmanaged action");
  });
});

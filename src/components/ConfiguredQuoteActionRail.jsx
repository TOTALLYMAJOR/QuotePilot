...
export default function ConfiguredQuoteActionRail({
  ...
  const primaryItems = normalizedPrimaryId
    ? actions.filter((item) => String(actionProp(item, "data-quote-action-id") || "").trim() === normalizedPrimaryId)
    : [];
  const secondaryItems = actions.filter((item) => (
  ...
  return (
    <div
      ...
      data-configured-quote-actions="v1"

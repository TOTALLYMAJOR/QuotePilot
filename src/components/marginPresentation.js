...
// Both staff shells compare the same recorded-cost evidence; this is not pricing authority.
export function buildRecordedCostMarginComparison({ isEditingQuote, editingQuote, catalog, effectiveSettings, proposedMargin }) {
  ...
  try {
    ...
    const currentMargin = buildMarginPresentation({ form: editingQuote.baseForm, totals: currentTotals, catalog, settings: effectiveSettings });
    ...
    return { evidenceState: "available", before: currentMargin.marginPct, proposedAfter: proposedMargin.marginPct };
  } catch {
    return {

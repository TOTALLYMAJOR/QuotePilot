...
export function parseReleaseUatArgs(argv) {
  ...
  if (argv[0] === "--print-items") {
    ...
    return { printItems: true, target: String(argv[2]).trim() };
  }
...
export function buildReleaseUatReceipt(
  ...
  const checkedItemIds = String(args["checked-item-ids"] || "")
    ...
    .filter(Boolean);
  const expectedItemIds = checklist.itemIdsByTarget[target];
  if (
    checkedItemIds.length !== expectedItemIds.length
    || new Set(checkedItemIds).size !== checkedItemIds.length
    || expectedItemIds.some((itemId) => !checkedItemIds.includes(itemId))
  ) {
    throw attestationError(
      `--checked-item-ids must contain every checklist item applicable to ${target} exactly once.`
    );
  }
  ...
  return Object.freeze({
    ...
    checklist: {
      ...
      checkedItemIds: expectedItemIds
    },
    github: {

...
async function signInAsStaff(page) {
  ...
  await expect(page.getByLabel("Event type", { exact: true })).toBeVisible({ timeout: 45_000 });
}
...
test("firebase auth and firestore rules load the organization catalog", async ({ page }) => {
  ...
  await expect(page.getByLabel("Event type", { exact: true }).locator("option")).toHaveCount(5);
});

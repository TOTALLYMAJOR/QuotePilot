...
| 24 | Tenant Import Studio (customer/catalog CSV recognition, validation, receipts, rollback) | Implemented (deployed; hosted smoke pending) | `src/components/ImportStudioModal.jsx`, `src/lib/importStudio.js`, `src/lib/importBatchService.js`, `firestore.rules` (`importBatches`) |
...
| 29 | Commercial Command Center ("Home"): triaged attention inbox, next-7-days accepted/booked events, and deposit/final-balance money glance, reusing existing workflow-attention and quote-history reads only; shared semantic status-chip system; staff-scoped neutral workspace chrome (portal/marketing branding unchanged) | Implemented (source) | `src/components/CommandCenterHome.jsx`, `src/components/StatusChip.jsx`, `src/lib/statusSemantics.js`, `src/App.jsx` (`workspaceView`, `app-shell-neutral`), `src/styles.css`, `src/lib/__tests__/statusSemantics.test.js`, `src/components/__tests__/commandCenterHome.test.jsx` |

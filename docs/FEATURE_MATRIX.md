...
| 27 | Staff Kitchen BEO export (timing, staffing, checkpoints, selections, checklist, exact source revision, download-time input digest, browser-local generated stamp, day-of contacts, allergens, prepared-by/chef sign-off, day-of notes) | Implemented (source; provenance is not retained freshness or receipt evidence; hosted operator acceptance pending) | `src/components/QuoteHistoryModal.jsx`, `src/lib/beoPayload.js`, `src/lib/beoArtifactFingerprint.js`, `src/lib/beoExport.js`, `src/lib/quoteWorkflow.js`, `src/lib/__tests__/beoPayload.test.js`, `src/lib/__tests__/beoArtifactFingerprint.test.js`, `src/lib/__tests__/beoExport.test.js`, `src/lib/__tests__/quoteWorkflow.test.js` |
...
| 33 | Commercial Dependency Graph v1 and Kitchen BEO download-time provenance: frozen validated registry, deterministic downstream traversal, `qp-canonical-json-v1` browser/Node SHA-256 parity, schema-bound declared-input fingerprint, exact source revision, and explicit non-freshness boundary | Implemented (source; CWF-15A only; focused local evidence; no persistence, receipt, freshness classification, simulation, or mutation; hosted operator acceptance pending) | `src/lib/commercialDependencyGraphCore.cjs`, `src/lib/commercialDependencyGraph.js`, `src/lib/beoArtifactFingerprint.js`, `src/lib/beoPayload.js`, `src/lib/beoExport.js`, `docs/COMMERCIAL_DEPENDENCY_GRAPH_ADR.md`, `src/lib/__tests__/commercialDependencyGraph.test.js`, `src/lib/__tests__/commercialDependencyGraph.parity.test.js`, `src/lib/__tests__/beoPayload.test.js`, `src/lib/__tests__/beoArtifactFingerprint.test.js`, `src/lib/__tests__/beoExport.test.js` |

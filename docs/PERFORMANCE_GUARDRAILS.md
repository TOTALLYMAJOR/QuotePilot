...
- A temporary exception, when present in
  ...
  `bundle-budget.json`.
- The current compatibility graph has a temporary 3,243,028-byte aggregate
  and 391,901-byte largest-chunk ceiling. The production-equivalent Ambient
  graph has a separate temporary 3,920,379-byte aggregate ceiling and
  ...
  The Event Profit Review candidate then measures 3,236,003 / 387,248 bytes for
  compatibility and 3,920,076 / 387,248 bytes for Ambient. Its recalibrated
  ceilings retain only those same 7,025-byte and 303-byte runner offsets; the
  largest-chunk ceiling is unchanged and no general growth headroom was added.

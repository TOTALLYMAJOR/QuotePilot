...
- The current compatibility graph has an exact temporary 2,791,699-byte
  ...
  Ambient graph has a separate temporary 3,723,748-byte aggregate ceiling and
  ...
  The quote-builder decision-flow slice then measures 2,791,568 bytes for the
  compatibility graph and 3,723,445 bytes for Ambient. Compatibility remains
  below its existing ceiling without adjustment. Ambient's reviewed ceiling is
  the exact local measurement plus only the already observed 303-byte runner
  offset; this is not general product-growth headroom. The largest chunk remains
  unchanged at 391,901 bytes in both profiles.

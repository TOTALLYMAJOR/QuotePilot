...
- The workspace-convergence exception (now including the flag-gated pilot
  ...
  event-shape memory), plus the source-only owner-SMS provider panel and its
  fail-closed Pingram evidence states, is
  currently capped at
  2,789,740
  ...
  CI bundle checks with one number. Fresh same-environment comparisons measured
  this branch at 2,789,740 bytes default-off and 2,789,726 bytes with all eight
  production flags, versus exact `v0.7.0` main at 2,776,981 and 2,776,967 bytes
  respectively. The owner-SMS surface therefore adds exactly 12,759 bytes in
  either configuration. Exact-main CI run `31528176575` measured the released
  tree at 2,776,793 bytes default-off and 2,776,849 bytes production-flag;
  applying the same source delta projects 2,789,552 and 2,789,608 bytes on the
  candidate CI runner. The 2,789,740 ceiling is the larger literal current
  contributor measurement, so it covers both profiles and the projected CI
  environment without discretionary growth headroom. Exact-SHA CI confirmation
  ...
  the unchanged clean-main baseline, aggregate output is 792,375 bytes
  (39.67%) larger, while

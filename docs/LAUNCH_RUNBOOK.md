...
v3 target/provider applicability in `docs/release-uat-checklist.json`; changes to that
...
For any release containing the owner-SMS provider slice, complete every
applicable `sms.*` item. First prove the disabled production floor remains
non-blocking. For a Pingram promotion, separately record the exact approved
regional origin, Secret Manager bindings without values, server-owned E.164
  owner destination, explicit consent, a new lowercase configuration
  generation, approved sender/A2P state, and exact
registered webhook endpoint. Then use one controlled owner destination to
observe request acceptance without calling it delivery, a valid signed
`delivered` or `failed` transition, replay deduplication, tamper rejection, and
  a callback-before-binding case that reprocesses, a signed unsubscribe/STOP
  hold, and an induced claimed or indeterminate result that stays locked without
  a second send. Browser-visible evidence must remain masked and the private
  outbox, attempts, provider binding, controls, rate limits, and webhook inbox must
remain browser-denied. Local tests do not satisfy this hosted/provider UAT.

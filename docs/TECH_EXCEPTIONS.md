...
- Change: Apply named, absolute temporary ceilings of 2,740,459 aggregate
...
- Risk impact: The emitted asset set is 743,094 bytes (37.20%) above the
  clean-main aggregate baseline, of which 49,115 bytes are the default-off
  ...
  and parser, 3,615 for the structured-record boundary and client, and 5,495 for the cascade receipts panel). A targeted `quoteStore` manual chunk reduces
...
- Performance impact: The August 10 merged-candidate build with the flag-gated
  pilot candidates emitted 2,740,459 aggregate JavaScript bytes and a
  ...
  band pricing strip; 2,721,338 before the client-request panel; 2,731,349 before the structured-record boundary; 2,734,964 before the cascade panel). The other largest emitted chunks were jsPDF at 385,630 bytes,

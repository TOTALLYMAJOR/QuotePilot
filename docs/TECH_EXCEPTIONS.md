...
- Change: Apply named, absolute temporary ceilings of 2,734,964 aggregate
...
- Risk impact: The emitted asset set is 737,599 bytes (36.93%) above the
  clean-main aggregate baseline, of which 43,620 bytes are the default-off
  ...
  and parser, and 3,615 for the structured-record boundary and client). A targeted `quoteStore` manual chunk reduces
...
- Performance impact: The August 10 merged-candidate build with the flag-gated
  pilot candidates emitted 2,734,964 aggregate JavaScript bytes and a
  ...
  band pricing strip; 2,721,338 before the client-request panel; 2,731,349 before the structured-record boundary). The other largest emitted chunks were jsPDF at 385,630 bytes,

...
- Change: Apply named, absolute temporary ceilings of 2,719,059 aggregate
...
- Risk impact: The emitted asset set is 721,694 bytes (36.13%) above the
  clean-main aggregate baseline, of which 27,715 bytes are the default-off
  ...
  cards, and 11,518 for the CREATE intake canvas and deterministic
  extractor). A targeted `quoteStore` manual chunk reduces
...
- Performance impact: The August 10 merged-candidate build with the flag-gated
  pilot candidates emitted 2,719,059 aggregate JavaScript bytes and a
  ...
  cards; 2,707,541 before the CREATE intake canvas). The other largest emitted chunks were jsPDF at 385,630 bytes,

...
9. Which manual assistive-technology/device matrix is required for acceptance: NVDA/Chrome, NVDA/Firefox, VoiceOver/Safari, Windows High Contrast, real 200%/400% zoom, switch control, and PWA install/relaunch?
10. Do first-time users understand `Start a quote`, guided CREATE compression, Proposal Composer blocker review, customer decision selection, attendance vocabulary, recovery outcomes, and compact inspector disclosures with real tenant data and production latency?
11. Should Product Truth Observability remain advisory after its first hosted CI observation, or become required after freshness and owner-comprehension review?
12. Which environment owner will supply each next missing evidence class: hosted, provider, production, human, or outcome?
